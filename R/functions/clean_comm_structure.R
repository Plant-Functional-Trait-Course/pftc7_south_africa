...
clean_comm_structure <- function(raw_structure){
  ...
  raw_structure |>
    ...
    mutate(unit = if_else(str_detect(variable, "height"), "cm", "percentage")) |> 
    select(date, aspect, site_id, elevation_m_asl = elevation, plot_id, treatment_warming = treatment_only_for_range_x, variable, value, unit)
